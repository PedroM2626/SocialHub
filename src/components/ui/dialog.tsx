...
const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & { full?: boolean }
>(({ className, children, ...props }, ref) => {
  // Helper to recursively check for a Dialog title in children
    ...
    for (const child of arr) {
      if (React.isValidElement(child)) {
        ...
        if (child.props && child.props.children) {
          if (containsDialogTitle(child.props.children)) return true
        }
      }
    }
    return false
  ...
  const isFull = Boolean((props as any).full)

  // remove custom prop before passing to primitive
  const { full, ...rest } = props as any
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          isFull
            ? 'fixed inset-0 z-50 grid gap-4 border bg-background p-6 shadow-lg duration-200 overflow-y-auto'
